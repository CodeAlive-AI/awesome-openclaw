...
## ⚠️ КРИТИЧЕСКИЕ ПРЕДУПРЕЖДЕНИЯ О БЕЗОПАСНОСТИ

**Прочитайте перед установкой!**

### Известные атаки и уязвимости Moltbot

| Угроза | Описание | Как защищает эта инструкция |
|--------|----------|----------------------------|
| **Обход аутентификации** | Сотни публичных инстансов взломаны через неправильную конфигурацию reverse proxy | ✅ Gateway только на loopback, нет публичного IP |
| **NPM squatting** | Пакет `moltbot` в npm — placeholder/вредонос, НЕ официальный | ✅ Используем официальный инсталлятор molt.bot |
| **VS Code malware** | Фейковое расширение "ClawdBot Agent" устанавливает RAT | ✅ Не требуется, не устанавливайте |
| **Supply chain атаки** | Вредоносные skills в MoltHub | ⚠️ Не устанавливайте непроверенные skills |
| **Prompt injection** | Через email можно выполнить произвольные команды | ⚠️ Не давайте боту доступ к email |

### Архитектурные проблемы (нельзя исправить конфигурацией)
...
- **Нет sandboxing** — агент не изолирован от системы

### Обязательные меры предосторожности

1. **НИКОГДА** не устанавливайте через `npm install -g moltbot`
2. **НИКОГДА** не открывайте Gateway порт (18789) в интернет
3. **НИКОГДА** не ставьте на основной рабочий компьютер
4. **ВСЕГДА** используйте изолированную VM
5. **ВСЕГДА** используйте одноразовые/отдельные API ключи
6. **РЕГУЛЯРНО** запускайте `clawdbot security audit`

### Источники информации о безопасности

- [Habr: Moltbot — разбор безопасности](https://habr.com/ru/news/989962/)
- [Habr: Security-аудит Clawdbot](https://habr.com/ru/articles/989764/)
- [Xakep: Скамеры эксплуатируют Moltbot](https://xakep.ru/2026/01/29/moltbot/)
